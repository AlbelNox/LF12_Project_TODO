...
type TodoCardListLinkProps = {
  todo: Todo;
  todoChecked?: (id: number, checked: boolean) => void;
  onNavigateToList?: (listId: number) => void;
  listId: number;
  listName?: string; // <-- Neu: Name der Liste als Prop
};

export function TodoCardListLink({
  todo,
  onNavigateToList,
  listId,
  listName, // <-- Neu: Name der Liste als Prop
}: TodoCardListLinkProps) {
  ...
  return (
    <Card
      className="mb-4 relative cursor-pointer hover:bg-gray-100"
      key={todo.id}
      ...
      <CardContent className="p-6">
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            ...
            {listName && (
              <span className="text-xs text-gray-500">{listName}</span>
            )}
          </div>
